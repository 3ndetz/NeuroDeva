...
if __name__ == "__main__":
    ...
    from other.HyperAI_DockerSecrets import DockerAuthKey # new, may cause errors, NOT TESTED!
    ...
    STT_Process.start()
    # откомментировать при включении! щас отключено!
    LLM_Process = Process(
        target=llm_process,
        args=(manager,))
    LLM_Process.start()
    print('Wait for loading LLM... (LLM DISABLED ДЛЯ УЧЕБЫ!)')
    manager.llm_loading_flag().wait() # ТОЖЕ ОТКОММЕНТИРОВАТЬ!
    #print('WAITING COMPLETED! BEZ LLM!')
